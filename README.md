...
4. On the laptop, confirm the `Form Shown In Photo` setting matches the actual uploaded paper; change it there and reread without re-uploading if the wrong form was chosen. For a radiology test photo shown inside a dark photo-viewer screen, Name Only attempts a guarded screen fit and keeps it only when the name reading improves; `Fit Screen Photo And Read Name` is available below the image. If an unusual photo still places the scan line incorrectly, choose `Select Name Area On Photo` and drag only across the printed name; Encodex OCRs only that marked name rectangle. For actual daily work, photograph the paper close-up instead of a screen. If an existing photo selected extra fields but you need only the written name, choose `Switch To Name Only And Read` to discard the extras and immediately read the name. Adjust alignment if needed and confirm only the selected fields.
