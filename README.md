...
3. Scan the QR code once with the phone and submit the first photograph. In continuous mode, tap `Capture Next Paper` after each successful upload to continue without rescanning a QR code.
   Photograph the physical paper directly and close enough for the printed name to be readable; a photo of the document displayed on another screen may be too low quality for reliable OCR.
   For an existing clear image already on the laptop, use `Use Laptop Image` in the source-image panel instead of photographing the screen.
4. On the laptop, confirm the `Form Shown In Photo` setting matches the actual uploaded paper; change it there and reread without re-uploading if the wrong form was chosen. For `Name Only`, choose `Find Name On Page`: Encodex reads the page locally long enough to locate the labeled name, then returns and stores only the name value. If it needs help, choose `Pick Name Row`, then click the middle of the printed patient name. Name OCR uses Tesseract locally and can retry with Windows offline OCR; no document is sent to a cloud OCR provider. For actual daily work, photograph the paper close-up instead of a screen. If an existing photo selected extra fields but you need only the written name, choose `Switch To Name Only And Read` to discard the extras and immediately read the name. Adjust alignment if needed and confirm only the selected fields.
5. For `Name Only`, verify the detected spelling and select `Review & Copy Name`; Encodex clearly confirms when the name is ready to paste into PhilHealth YAKAP. Unreliable OCR name text is rejected rather than offered for copying; type a correction or select the printed name line again. For other profiles, use each confirmed field's copy button or explicitly export reviewed fields to CSV.
...
- `Name Only` may transiently read a typed page locally to locate the `NAME:` label, but only the requested name is returned, stored, copied, or exported. Other selected typed fields are handled locally, and medical-certificate handwriting is manual-first.
